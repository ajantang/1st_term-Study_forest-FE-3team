...
export function Modal({ studyName, isOpen, onClose, modalType }) {
  ...
  const [toggleVisibleClass, setToggleVisibleClass] = useState('modal__password-input-visible-off');
  ...
  const wrongPasswordLengthWarn = () => {
    const warning = (
      <div className="flex-row modal__warning">
        <p className="font16 medium modal__warning-text">🚨 8 ~ 24 자리의 비밀번호를 입력해주세요.</p>
      </div>
    );
        ...
        <div className="modal__password">
          ...
          <div className="modal__password-inputset">
            <input
              ...
              type={inputType}
            ></input>
            <svg onClick={toggleVisiblePassword} className={toggleVisibleClass} />
          </div>
        </div>
        <svg className={buttonClass[modalType]} onClick={onModalClick} />
        <p className="font16 medium modal__btn-close" onClick={handleModalClose}>
          나가기
        </p>
      </div>

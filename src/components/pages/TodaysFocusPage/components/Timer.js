...
import { updatePoint } from '../../../../api/pointApi.js';
...
const Timer = ({ initialPoint, setPoint, setAlertGetPoint }) => {
  ...
  const StartTimer = () => {
    ...
    // 일시정지 버튼 동적 생성
    setPauseTimer(<button type="button" className="timer__controls__pause" onClick={PauseTimer}></button>);

    // 초기화 버튼 동적 생성
    setClearTimer(<button type="button" className="timer__controls__reset" onClick={ClearTimer}></button>);
    ...
    timer = setInterval(() => {
      ...
      if (count >= 0) {
        ...
        // 정지 버튼 동적 생성
        setControltimer(
          <button type="button" id="stoptbtn" className={`timer__controls__active extra-bold stop`} onClick={StopTimer}>
            &nbsp;&nbsp; Stop!
          </button>
        );
        if (count >= -60) {
  ...
  const StopTimer = () => {
    ...
    setAlertGetPoint(<div className="condition__alert points font16 medium">🎉 {obtainPoint}포인트를 획득했습니다!</div>);
  };
  ...
  const PauseTimer = () => {
    ...
    setState('pause');
    setIsDisableBtn(false);
    setAlertCondition(<div className="condition__alert pause font16 medium">🚨 집중이 중단되었습니다.</div>);
    clearInterval(timer);
  };
